    ...
    render() {
      ...
      return (
        <Modal
          visible={visible}
          width='60%'
          title="创建测试（注：请任务负责人在任务完成后将相关文档以任务名_人名_时间的命名方式上传到文件管理）"
          okText="创建"
          cancelText="取消"
          onCancel={onCancel}
          onOk={onOk}
...
class Task_3 extends Component{
  ...
  handleOk(){
    ...
    form.validateFields((err, values) => {
      ...
      // console.log(this.props.taskData.getIn(['bigTasks',3,'id']))
      ...
      axios.post(localStorage.api+'team/addSmallTask',qs.stringify({
        ...
        desc:values['desc'],
        // itemId:this.props.taskData.bigTasks[0].id,
        itemId:this.props.taskData.getIn(['bigTasks',3,'id']),
        endTime:datetime,
      }),{withCredentials:true}
      )
      .then(function(response){
  ...
  submitTask(){
    ...
    let judge = this.props.taskData.getIn(['bigTasks',3,'smallTasks']).toJS().every((value,index) => {
      // console.log("判断完成")
      // console.log(value)
    ...
    if(judge){
      ...
      if(this.props.taskData.getIn(['bigTasks',3,'status']) == '已完成'){
        message.error('请勿重复提交！')
      }else{

      axios.post(localStorage.api+'team/bigStatus',qs.stringify({
        taskId:this.props.taskData.getIn(['bigTasks',3,'id']),
        status:'已完成'
      }),{withCredentials:true}
      )
      .then(function(response){
      ...
      <Fragment>
        <Row>
          <Col className="task-title" span = {6}>
            ...
            <span className="task-name"> — { this.props.taskData.getIn(['bigTasks']) && this.props.taskData.getIn(['bigTasks',3,'workerName'])  ?  this.props.taskData.getIn(['bigTasks',3,'workerName']) : "未分配" }</span>
          </Col>
          <Col span = {6}></Col>
          ...
          <Col span = {6} style={{textAlign:'right'}}>
            {
                this.props.personal.identity.contains("组长") || this.props.personal.identity.contains("测试负责人") == true 
            ...
            {
                this.props.personal.identity.contains("组长") || this.props.personal.identity.contains("测试负责人") == true 
                ?<Button onClick={this.newTask.bind(this)} className="task-new">新建测试</Button>
                :<Button onClick={this.newTask.bind(this)} className="task-new" disabled>新建测试</Button> 
            }
...
export default connect(mapStateToProps,mapDispatchToProps)(Task_3)
