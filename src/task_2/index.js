              ...
              <Form.Item label="任务状态">
                {getFieldDecorator('station', {
                  ...
                  >
                    ...
                    {
                      personal.identity.contains("组长") || personal.identity.contains("实现负责人") == true ?<Option key = "3" value="已完成" >已完成</Option>:null
                    }
    ...
    render() {
      ...
      return (
        <Modal
          visible={visible}
          width='60%'
          title="创建实现（注：请任务负责人在任务完成后将相关文档以任务名_人名_时间的命名方式上传到文件管理）"
          okText="创建"
          cancelText="取消"
          onCancel={onCancel}
          onOk={onOk}
  ...
  render(){
    return(
      <Fragment>
        ...
        <Table rowKey="id" className="task-table" columns={columns} dataSource = {this.props.taskData.getIn(['bigTasks']) == null ? null : this.props.taskData.getIn(['bigTasks',2,'smallTasks']).toJS() }  pagination={setting}         
           onRow={(record,rowkey)=>{
             return{
